        ...
        "nodes": {
            ...
            "items": {
                ...
                "properties": {
                    ...
                    "form_fields": {
                        "type": "array",
                        "description": "Campos del formulario que se llenaran en esta actividad.",
                        "items": {
                            "type": "object",
                            "required": ["key", "label", "field_type", "required", "options"],
                            "properties": {
                                "key": {"type": "string"},
                                "label": {"type": "string"},
                                "field_type": {
                                    "type": "string",
                                    "enum": ["texto", "numero", "fecha", "lista", "archivo", "imagen", "booleano"],
                                },
...
def _build_prompt(payload: WorkflowSuggestionRequest, transcript: str | None = None) -> str:
    context_lines = [
        ...
        "Asume que el usuario NO es experto: aunque escriba poco, completa el proceso con buenas practicas de negocio.",
        ...
        "Si el proceso implica atencion al cliente, agrega campos de datos del solicitante, observaciones y prioridad.",
        ...
        'Ejemplo de salida valida: {"title":"Flujo de Cotas","summary":"Resumen breve","nodes":[{"code":"INI","name":"Inicio","node_type":"inicio","lane":"Sistema","form_fields":[]},{"code":"ATC","name":"Registrar solicitud","node_type":"actividad","lane":"Atencion al Cliente","form_fields":[{"key":"nombre_solicitante","label":"Nombre del solicitante","field_type":"texto","required":true,"options":[]}]},{"code":"DOCOK","name":"Documentos completos?","node_type":"decision","lane":"Atencion al Cliente","form_fields":[]},{"code":"FIN","name":"Fin","node_type":"fin","lane":"Sistema","form_fields":[]}],"transitions":[{"source_code":"INI","target_code":"ATC","transition_type":"secuencial"},{"source_code":"ATC","target_code":"DOCOK","transition_type":"secuencial"},{"source_code":"DOCOK","target_code":"FIN","transition_type":"alternativa","condition_label":"Si"}]}',
    ]
...
def _fallback_workflow_suggestion(payload: WorkflowSuggestionRequest) -> WorkflowSuggestionResponse:
    ...
    return WorkflowSuggestionResponse(
        ...
        model=settings.gemini_model,
        title=title,
        summary="Se genero un flujo UML base enriquecido con validacion documental, decisiones, paralelismo y cierre.",
        nodes=[
            {"code": "INI", "name": "Inicio", "node_type": "inicio", "lane": "Sistema", "form_fields": []},
            {
                "code": "ATC",
                ...
                "responsible_department": "Atencion al Cliente",
                "form_fields": [
                    {"key": "nombre_cliente", "label": "Nombre del cliente", "field_type": "texto", "required": True, "options": []},
                    {"key": "documento_cliente", "label": "Documento del cliente", "field_type": "texto", "required": True, "options": []},
                    {"key": "descripcion_solicitud", "label": "Descripcion de la solicitud", "field_type": "texto", "required": True, "options": []},
                ],
            },
            {
                "code": "DOC",
                "name": "Cargar documentacion",
                "node_type": "actividad",
                "lane": "Atencion al Cliente",
                "responsible_role": "funcionario",
                "responsible_department": "Atencion al Cliente",
                "form_fields": [
                    {"key": "documento_respaldo", "label": "Documento de respaldo", "field_type": "archivo", "required": True, "options": []},
                    {"key": "observaciones_recepcion", "label": "Observaciones de recepcion", "field_type": "texto", "required": False, "options": []},
                ],
            },
            {
                "code": "DOCOK",
            ...
            {
                "code": "SUB",
                "name": "Solicitar subsanacion",
                "node_type": "actividad",
                "lane": "Atencion al Cliente",
                "responsible_role": "funcionario",
                "responsible_department": "Atencion al Cliente",
                "form_fields": [
                    {"key": "motivo_subsanacion", "label": "Motivo de subsanacion", "field_type": "texto", "required": True, "options": []},
                    {"key": "fecha_limite", "label": "Fecha limite", "field_type": "fecha", "required": True, "options": []},
                ],
            },
            {"code": "FORK", "name": "Dividir revisiones", "node_type": "fork", "lane": "Sistema", "form_fields": []},
            {
                "code": "REVTEC",
                "name": "Revision tecnica",
                "node_type": "actividad",
                "lane": "Operacion",
                "responsible_role": "analista",
                "responsible_department": "Operacion",
                "form_fields": [
                    {"key": "resultado_tecnico", "label": "Resultado tecnico", "field_type": "lista", "required": True, "options": ["viable", "observado", "no viable"]},
                    {"key": "riesgo_demora", "label": "Riesgo de demora", "field_type": "lista", "required": True, "options": ["bajo", "medio", "alto"]},
                ],
            },
            {
                "code": "REVADM",
                "name": "Revision administrativa",
                "node_type": "actividad",
                "lane": "Administracion",
                "responsible_role": "supervisor",
                "responsible_department": "Administracion",
                "form_fields": [
                    {"key": "cumple_normativa", "label": "Cumple normativa", "field_type": "booleano", "required": True, "options": []},
                    {"key": "observacion_administrativa", "label": "Observacion administrativa", "field_type": "texto", "required": False, "options": []},
                ],
            },
            {"code": "JOIN", "name": "Consolidar revisiones", "node_type": "join", "lane": "Sistema", "form_fields": []},
            {
                "code": "APR",
            ...
            {
                "code": "EJE",
                "name": "Ejecutar atencion",
                "node_type": "actividad",
                "lane": "Operacion",
                "responsible_role": "funcionario",
                "responsible_department": "Operacion",
                "form_fields": [
                    {"key": "fecha_ejecucion", "label": "Fecha de ejecucion", "field_type": "fecha", "required": True, "options": []},
                    {"key": "evidencia_cierre", "label": "Evidencia de cierre", "field_type": "archivo", "required": False, "options": []},
                ],
            },
            {"code": "RECH", "name": "Notificar rechazo", "node_type": "actividad", "lane": "Atencion al Cliente", "responsible_role": "funcionario", "responsible_department": "Atencion al Cliente", "form_fields": [{"key": "motivo_rechazo", "label": "Motivo de rechazo", "field_type": "texto", "required": True, "options": []}]},
            {"code": "FIN", "name": "Fin", "node_type": "fin", "lane": "Sistema", "form_fields": []},
        ],
        transitions=[
            {"source_code": "INI", "target_code": "ATC", "transition_type": "secuencial"},
            {"source_code": "ATC", "target_code": "DOC", "transition_type": "secuencial"},
            {"source_code": "DOC", "target_code": "DOCOK", "transition_type": "secuencial"},
            {"source_code": "DOCOK", "target_code": "SUB", "transition_type": "alternativa", "condition_label": "No"},
            {"source_code": "SUB", "target_code": "DOC", "transition_type": "iterativa", "condition_label": "Subsanado"},
            {"source_code": "DOCOK", "target_code": "FORK", "transition_type": "alternativa", "condition_label": "Si"},
            {"source_code": "FORK", "target_code": "REVTEC", "transition_type": "paralela"},
            {"source_code": "FORK", "target_code": "REVADM", "transition_type": "paralela"},
            {"source_code": "REVTEC", "target_code": "JOIN", "transition_type": "paralela"},
            {"source_code": "REVADM", "target_code": "JOIN", "transition_type": "paralela"},
            {"source_code": "JOIN", "target_code": "APR", "transition_type": "secuencial"},
            {"source_code": "APR", "target_code": "EJE", "transition_type": "alternativa", "condition_label": "Aprobado"},
            {"source_code": "APR", "target_code": "RECH", "transition_type": "alternativa", "condition_label": "Rechazado"},
            {"source_code": "EJE", "target_code": "FIN", "transition_type": "secuencial"},
            {"source_code": "RECH", "target_code": "FIN", "transition_type": "secuencial"},
        ],
    )
